...
void ft_load(t_pc *pc, t_union *un)
{
	...
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 2, 2, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_st(t_pc *pc, t_union *un)
{
	...
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 2, 3, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_and(t_pc *pc, t_union *un)
{
	...
	unsigned int two;
	int plus;
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 3, 6, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_or(t_pc *pc, t_union *un)
{
	...
	unsigned int two;
	int plus;
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 3, 7, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_xor(t_pc *pc, t_union *un)
{
	...
	unsigned int two;
	int plus;
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 3, 8, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_ldi(t_pc *pc, t_union *un)
{
	...
	int two;
	plus = 0;
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 3, 10, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_sti(t_pc *pc, t_union *un)
{
	...
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 3, 11, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_lld(t_pc *pc, t_union *un)
{
	unsigned int n;
	int plus;
	if (!ft_check_codage(un->map[pc->curr_position + 1].value, 2, 13, un))
	{
		pc->curr_position += 2;
		return ;
	}
...
void ft_lldi(t_pc *pc, t_union *un)
{
	...
	int one;
	int two;
	if(!ft_check_codage(un->map[pc->curr_position + 1].value, 3, 14, un))
	{
		pc->curr_position += 2;
		return ;
	}
